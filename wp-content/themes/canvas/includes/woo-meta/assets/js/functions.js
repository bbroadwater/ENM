...
jQuery( 'form#wooform input.reset-button' ).click( function () {
	...
	var confirmed = confirm( 'Are you sure you want to reset these options? All customised meta will be lost!' );
...
----------------------------------------*/
	...
	jQuery( 'form#wooform' ).submit( function ( e ) {
		...
		switch ( logicType ) {
			...
			case 'reset':
				formData = '&woometa_reset=true&' + formData;
				formAction = formAction.replace( 'updated=true', 'reset=true' );
			break;
			
			case 'save':
				formData = '&woometa_update=true&' + formData;
			break;
		...
		if ( formAction ) {
			...
			var jqxhr = jQuery.post( formAction, formData, function( response, textStatus, jqXHRObj ) {
				...
				if ( logicType == 'reset' ) {
					...
					jQuery( '#woo_container #content' ).fadeTo( 'slow', 0.5, function () {
						var newContent = jQuery( response ).find( '#content' );
						
						jQuery( this ).html( jQuery( newContent ).html() ).fadeTo( 'slow', 1 );
					});
