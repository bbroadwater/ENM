					...
					<?php if ($the_query->have_posts()) : while ($the_query->have_posts()) : $the_query->the_post(); ?>
						<a href="<?php the_permalink();?>"<?php if($i==0) { ?> class="active"<?php } ?>>
							<span class="slider-overlay">
								<strong><?php the_title();?></strong> <strong style="font-size:14px;">By  <?php the_author(); ?></strong>
								<span>
									<?php 
										//add_filter('excerpt_length', 'new_excerpt_length_0');
										//the_excerpt();
									?>
								</span>
							</span>
				...
				<ul class="slider-navigation">
					...
					<?php if ($the_query->have_posts()) : while ($the_query->have_posts()) : $the_query->the_post(); ?>
						<li<?php if($i==0) { ?> class="active"<?php } ?>>
							<a href="#" data-target="1" onclick="location.href = '<?php the_permalink(); ?>';">
                                                                <strong><?php the_title();?></strong><strong style="font-size:10px; padding:0 15px 0;">By  <?php the_author(); ?></strong>
								<span>
                                									<?php 
