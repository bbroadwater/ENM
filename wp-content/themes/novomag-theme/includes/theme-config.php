...
';
			...
			if(isset($value['page_type']) && ot_page_type_check($value['page_type'])==true || !isset($value['page_type'])) { //meta box check
				...
				?>
						<div class="row-explenation">
							<span><?php _e("This options is disabled, to enable it you must set this option in <strong>theme management panel</strong> to custom.");?></span>
						</div>
				<?php
					}		
				}		
	...
	*/
	function print_add_text($value) {
		...
		<a href="javascript:{}" onclick="document.getElementById('orange-themes-options').submit(); return false;" class="button-1">Add</a>
	...
	function print_scroller($value) {
		...
		<?php if( $value['home'] == "yes" ) { ?><div class="input-item-full-width-inside clearfix"><?php } ?>
			...
			<span class="<?php echo $value['id'];?>" id="scroller">
				<input name="<?php echo $value['id'];?>" value="<?php echo $input_value;?>" class="scroller" type="text" id="amount_<?php echo $value['id'];?>" style="border:0; color:#f6931f; font-weight:bold; width: 50px;" readonly="readonly"/>
