 ...
 * -------------------------------------------------------------------------*/
			...
			drop: function(event, ui) {
				...
				if (!ui.draggable.hasClass("dropped")) {
					...
					jQuery("#recordsArray_"+randomID+" input,#recordsArray_"+randomID+" textarea,#recordsArray_"+randomID+" select,#recordsArray_"+randomID+" div.uploader span").each(function () {
						...
						if(jQuery(this).hasClass("btn-upload")) {
						...
						} else if(jQuery(this).hasClass("scroller")){
							...
							if((typeof(originalName) != "undefined" && originalName !== null) && originalName.indexOf("[]") != -1) {
	...
	jQuery(document).ready(function($){
		jQuery(".dropped div a").live("click", function(){
 ...
 * -------------------------------------------------------------------------*/	
 jQuery(document).ready(function() {  
 	...
 	jQuery("#ot-submit-home").live("click", function(){
 		jQuery("#ot-submit-home").attr("value","Saving...");
 		var adminUrl = scripts.adminUrl;
 		var postId = jQuery("#post_ID").val();

		var order = jQuery("ul#active-homepage-blocks *").serialize()+"&action=ot_save_options";
		jQuery.post(adminUrl,order, function(theResponse){

			//alert(theResponse);
			//alert(order);
		}).done(function() { 
			jQuery("#ot-submit-home").attr("value","Saved");
			setTimeout(function(){
        		jQuery("#ot-submit-home").attr("value","Save");
        	}, 2000);
